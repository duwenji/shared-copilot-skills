...
Provide a reusable, agent-friendly build workflow for markdown repositories that follow numbered chapter conventions and need EPUB, PDF, and KDP registration Markdown artifacts.
...
7. Copy the generated artifacts to `outputDir` using `projectName` as the filename base.
...
- `pdf`: generated via an HTML print pass and a local Chrome/Edge renderer for a fixed-layout style output.
