...
Provide a reusable, agent-friendly build workflow for markdown repositories that follow numbered chapter conventions and need EPUB, PDF, fixed-name `cover.pdf` / `cover.jpg`, and KDP registration Markdown artifacts.
...
7. Copy the generated artifacts to `outputDir` using `projectName` as the filename base; when `pdf` is requested, also emit fixed-name `cover.pdf` and `cover.jpg` in the same folder.
...
- `pdf`: generated via an HTML print pass and a local Chrome/Edge renderer for a fixed-layout style output. When requested, the workflow also emits `cover.pdf` and `cover.jpg` to `outputDir` using fixed filenames.
