...
7. Copy the generated artifacts to `outputDir` using `projectName` as the filename base; when `pdf` is requested, also emit fixed-name `cover.pdf` and `cover.jpg` in the same folder. `cover.pdf` is sized as a paperback cover spread using the trim size plus a spine width derived from the generated page count.
